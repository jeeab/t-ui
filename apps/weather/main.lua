...
-- Uses the GPS for your location and Wi-Fi to fetch. Set up Wi-Fi in Settings first,
-- and give the GPS a moment to get a fix (needs a view of the sky).
--
-- Tap the screen to refresh. Bringing Wi-Fi up drops Bluetooth until the next reboot.
...
local state = "idle" -- idle / fetching / ok / error / nogps / nowifi / oldfw
...
local function draw()
  ...
  elseif state == "nogps" then
    show("Waiting for a GPS fix...", AMBER, "Needs a view of the sky. Tap to retry.", DIM, "")
...
local function refresh()
  ...
  if not net.fetch(url) then
    -- false = no Wi-Fi network saved (or a fetch is somehow already running)
...
function on_open()
  draw()
  refresh() -- try once on open; the user can tap to retry once GPS/Wi-Fi are ready
end
...
function on_tick()
  ...
  local s = net.status()
  if s == "done" then
    local body = net.body() -- reading clears the result
    if body then
      ...
      desc = codeDesc(code)
      if temp then
        temp = tostring(math.floor(tonumber(temp) + 0.5)) -- whole degrees
